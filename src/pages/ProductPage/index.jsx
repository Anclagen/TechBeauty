    ...
    <Wrapper>
      ...
      <PageGrid>
        ...
        <DetailsContainer>
          ...
          <Divider/>
          {data.discountedPrice < data.price ? <PreviousPrice>Original Price: <PreviousPriceAmount>kr {data.price}</PreviousPriceAmount></PreviousPrice>: ""}
          ...
          <Divider/>
          <ButtonAddCart onClick={() => dispatch(addToCart({quantity:1, product:data}))}><IconImage src={cartIcon} alt="cart icon"/>Add To Cart</ButtonAddCart>
          <Divider/>
      ...
      <div>
        {data.reviews && data.reviews.length > 0 ? data.reviews.map((review) => <div key={review.id}>
          <Divider/>
