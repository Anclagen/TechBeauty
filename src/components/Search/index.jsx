...
export default function Search(){
  ...
  return(
    <SearchContainer>
      ...
      <AutoComplete>
        {results.length > 0 ? 
          <ResultContainer> {results.map(item => <SearchItem key={item.id} item={item}/>)}</ResultContainer> : 
          query.length > 1 && query.trim() !== ""? <NoResult>No results found for "{query}"</NoResult> : ""
        }
      </AutoComplete>
    </SearchContainer>
  )
